      ...
      {selectedLocation ? (
        <div className="space-y-3">
          ...
          <div className="h-48 w-full rounded-xl overflow-hidden border border-surface-200 shadow-sm relative z-0">
            {isSearching && (
              <div className="absolute inset-0 bg-surface-50/50 backdrop-blur-sm z-[1000] flex items-center justify-center">
        ...
        <div className="space-y-4 animate-in fade-in duration-200">
          <Input
            ...
            prefixNode={<Search size={18} />}
            suffixNode={
              searchQuery ? (
                <button
                  type="button"
                  onClick={() => setSearchQuery('')}
                  className="w-full h-full flex items-center justify-center text-surface-400 hover:text-surface-600 transition-colors focus:outline-none focus-visible:text-brand-500 rounded-lg focus-visible:ring-2 focus-visible:ring-brand-500"
                >
                  <X size={16} />
                </button>
              ) : undefined
          ...
          {!searchQuery.trim() ? (
            <div className="space-y-2">
              <button
                ...
                className="w-full flex items-center gap-3 p-3 bg-surface-0 border border-surface-200 rounded-xl hover:bg-brand-50 hover:border-brand-200 transition-all active:scale-[0.98] disabled:opacity-50 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
              >
            ...
            <div className="bg-surface-0 border border-surface-200 rounded-xl shadow-sm overflow-hidden divide-y divide-surface-100 min-h-[100px] relative">
              {isSearching ? (
