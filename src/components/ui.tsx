...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ className, prefixNode, suffixNode, error, prefixWidth, suffixWidth, ...props }, ref) => {
    return (
      <div 
        className={`flex items-stretch w-full bg-surface-0 border rounded-xl transition-all focus-within:ring-2 focus-within:ring-brand-500/30 focus-within:border-brand-500 group overflow-hidden ${
          error ? 'border-red-300 focus-within:border-red-500 focus-within:ring-red-500/30' : 'border-surface-200'
        } ${className || ''}`}
      >
        {prefixNode && (
          <div className="flex items-center justify-center pl-3.5 pr-2 text-surface-400 group-focus-within:text-brand-500 flex-shrink-0">
            {prefixNode}
          </div>
        )}
        <input
          ref={ref}
          className={`flex-1 bg-transparent text-sm text-surface-900 placeholder:text-surface-400 focus:outline-none py-3 min-w-0 ${
            !prefixNode ? 'pl-4' : 'pl-0'
          } ${!suffixNode ? 'pr-4' : 'pr-0'}`}
          {...props}
        />
        {suffixNode && (
          <div className="flex items-center justify-center pr-3.5 pl-2 text-surface-400 flex-shrink-0">
            {suffixNode}
          </div>
        )}
      </div>
