...
for (let file of files) {
  ...
  for (let m of sorted) {
    if (content.includes(m)) {
      dryRunStr += '[' + file + '] ' + JSON.stringify(m) + ' -> ' + map[m] + '\n';
      content = content.split(m).join(map[m]);
      changed = true;
    }
  }
  if (changed) {
    fs.writeFileSync(file + '.fixed', content, 'utf8'); // Dry run save
  }
}
fs.writeFileSync('dryrun.log', dryRunStr);
console.log('Dry run complete. Check dryrun.log');